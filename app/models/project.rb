...
class Project < ApplicationRecord
  ...
  def attrs_recursive
    attributes.slice('id', 'status', 'archived', 'title', 'project_id').merge(
      tasks: tasks.map { |t| t.attrs_recursive },
      memberships: memberships.map { |m| m.attrs },
      users: memberships.map { |m| m.user.attrs }
    )
  end

  # returns minimal list of attributes (for index view)
  def attrs_recursive_brief
    attributes.slice('id', 'status', 'visibility', 'archived', 'title', 'visibility', 'due_date', 'project_id').merge(
